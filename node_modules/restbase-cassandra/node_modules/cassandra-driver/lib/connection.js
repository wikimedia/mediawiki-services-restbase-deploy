...
Connection.prototype.chainEvents = function() {
  ...
  this.netClient.on('timeout', function() {
    //there is a TCP connection that should be killed.
    var err = new Error('Connection to ' + self.address + ':' + self.port + ' timed out');
    self.netClient.destroy(err);
    // Destroy implicitly emits the 'close' event, so don't call the close
    // handler explicitly.
  });
  this.netClient.on('close', function (err) {
    self.log('info', 'Connection to ' + self.address + ':' + self.port + ' closed');
    self.connected = false;
    self.connecting = false;
    self.invokePendingCallbacks(err);
  });
...
Connection.prototype.open = function (callback) {
  ...
  // Speed up failure detection with TCP keep-alives
  this.netClient.setKeepAlive(true, this.options.socketOptions.connectTimeout);
};
